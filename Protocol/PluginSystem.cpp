    ...
    if (DevMode)
    {
        ...
        while ((dirent = readdir(dir)) != NULL)
        {
            ...
            if (dirent->d_type == DT_DIR)
            {
                ...
                if (d.HasParseError())
                {
                    Log::AddLog(Log::LogType::_ERROR, Log::MsgType::PROGRAM, u8"LoadPlugin", u8"Parse Json fail, Appid:%d, ParseErrorCode:%d, ErrorOffset:%ul",true, dirent->d_name, d.GetParseError(), d.GetErrorOffset());
                    return;
                }
